  ...
  <div>
    ...
    >
      <form name="playerForm" @submit="submitPlayerForm">
        ...
        <b-form-group v-if="mainImage">
          <img :src="mainImage | serverimage" width="150" height="auto" />
        </b-form-group>
  ...
  methods: {
    submitPlayerForm(e) {
      ...
      if (
        ...
        this.playerDetail.team = this.teamId;
        service.savePlayer(this.playerDetail, result => {
          if (result.data) {
            this.errorCheck = false;
            this.$toasted.success("Member details are added successfully");
            this.$router.go(0);
          } else {
            this.errorCheck = true;
            this.errMessage = "Retry adding the member.";
          }
        });
      } else {
        this.errorCheck = true;
        ...
        if (filename.lastIndexOf(".") <= 0) {
          ...
          service.upload(formData, data => {
            if (data.data.data) {
              ...
              this.mainImage = data.data.data[0];
              this.playerDetail.photograph = this.mainImage;
            }
          });
        }
      } else {
